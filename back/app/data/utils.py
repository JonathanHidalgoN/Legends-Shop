...
from app.data.queries.itemQueries import getAllEffectNamesAndValueAssociatedByItemId, getAllStatNamesAndValueAssociatedByItemId, getAllTagNamesAssociatedByItemId, getGoldTableWithId, getItems
...
async def convertItemTableIntoItem(asyncSession : AsyncSession, itemTable: ItemTable) -> Item:
    """
    This function takes an itemTable and get all the relations wiht gold,effects,stats and tags.
    Then creates an Item object
    """
    goldTable : GoldTable | None = await getGoldTableWithId(asyncSession,itemTable.gold_id) 
    ...
    effectsData : Dict[str, int | float]  = await getAllEffectNamesAndValueAssociatedByItemId(
        asyncSession, itemTable.id
    )
    stats:Stats = Stats(root=statsData)
    effects : Effects =  Effects(root=effectsData)
    image : Image = Image(
        full=itemTable.image,
        sprite="",
        group=""
    )
