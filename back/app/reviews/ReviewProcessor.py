...
from app.data.queries.reviewQueries import (
    addReview,
    getReviewsByUserId,
    getCommentsByUserId,
)
...
from app.data.queries.reviewQueries import getReviewsWithCommentsByItemId
...
class ReviewProcessor:
    ...
    async def addReviewAndComments(self, review: Review, userId: int) -> None:
        try:
            ...
            reviewId: int = await addReview(
                self.dbSession, review.orderId, review.itemId, review.rating
            )
    ...
    async def updateReviewAndComments(self, review: Review, userId: int) -> None:
        ...
        try:
            ...
            await updateReview(self.dbSession, review.id, review.rating)
            ...
            if review.comments and len(review.comments) > 0:
                ...
                existing_comments = await getCommentsByReviewId(
                    self.dbSession, review.id
                )

                if existing_comments:
                    # Update existing comment
                    await updateComment(
                        self.dbSession, existing_comments[0].id, comment_content
                    )
                else:
                    # Add new comment
                    await addComment(self.dbSession, review.id, userId, comment_content)
    ...
    @logMethod
    async def getReviewsAndCommentsByItemId(self, itemId: int) -> List[Review]:
        """
        Get all reviews with their comments for a specific item.

        Args:
            itemId (int): The ID of the item whose reviews to retrieve

        Returns:
            List[Review]: A list of reviews with their associated comments
        """
        try:
            reviews = await getReviewsWithCommentsByItemId(self.dbSession, itemId)
            mapped_reviews = [mapReviewTableToReview(review) for review in reviews]

            return mapped_reviews
        except SQLAlchemyError as e:
            raise ReviewProcessorException("Error retrieving item reviews") from e
