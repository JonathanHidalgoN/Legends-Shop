...
async def test_getUserGoldWithUserName_success(worker: ProfileWorker):
    ...
    with patch("app.profile.ProfileWorker.getCurrentUserGoldWithUserName", new=AsyncMock(return_value=expectedGold)):
...
async def test_getUserGoldWithUserName_no_gold(worker: ProfileWorker):
    ...
    with patch("app.profile.ProfileWorker.getCurrentUserGoldWithUserName", 
